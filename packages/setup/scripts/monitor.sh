...
show=`echo "$value" |sed -e 's/^/       /'`
        ...
        case $returncode in
        	1)
                ...
                --yesno "$(gettext "Are you sure to quit?")" 10 30
                case $? in
                0)
                        exit
                        ;;
